...
path &pathmanager::at( unsigned int index ) {
	return m_paths[ index ];
}

std::vector<path> &pathmanager::paths() {
	return m_paths;
}

std::vector<path> pathmanager::filter_bounding_box( rect bounding_box ) {
	std::vector<path> paths;

	for ( auto& path : m_paths ) {
		if ( path.inside( bounding_box )) {
